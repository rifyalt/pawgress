...
def page_xp_control():
    cl,cr=st.columns([3,2])
    with cl:
        st.markdown("#### ⭐ XP Pending Approval")
        if not task_df.empty and "QC Status" in task_df.columns:
            pending=task_df[(task_df["Status"]=="Done")&(task_df["QC Status"]=="Pending QC")]
            if pending.empty: st.info("Tidak ada XP pending. 🎉")
            for idx,row in pending.head(20).iterrows():
                ...
                with st.expander(f"{sn} — {task_r}  ·  +{xp_r} XP"):
    ...
    with cr:
        ...
        with st.form("manual_xp_form"):
            ...
            xtyp=st.selectbox("Jenis",["Bonus XP Manual","Penalti XP","Hold XP"])
            amt =st.number_input("Jumlah XP",1,500,50)
        ...
        if sub:
            ...
            load_data.clear(); st.session_state.toast_msg=f"{'Bonus' if sign>0 else 'Penalti'} {amt} XP → **{tgt}**."; st.rerun()
